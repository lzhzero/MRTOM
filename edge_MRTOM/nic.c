 ...
 */
static const struct rte_eth_conf conf_default = {
    .rxmode =
        {
        //    .split_hdr_size = 0,
...
static const struct rte_eth_txconf tx_conf_default = {
    .tx_thresh = {.pthresh = 0, .hthresh = 0, .wthresh = 0},
    .tx_rs_thresh = 0,
    .tx_free_thresh = 0,
    .tx_deferred_start = 0,};
 ...
 */
static int init_port(const uint8_t port_id, struct rte_mempool *mem_pool,
                     const uint16_t nb_rx_queue, const uint16_t nb_rx_desc)
{


    struct rte_eth_dev_info dev_info;
    rte_eth_dev_info_get(port_id, &dev_info);
    printf("dev info is %lu\n", dev_info.tx_offload_capa);
    printf("max burst size is %u\n", dev_info.default_txportconf.burst_size);
    struct rte_eth_conf conf = conf_default;
    ...
    if (nb_rx_queue > dev_info.max_rx_queues)
    {
        rte_exit(EXIT_FAILURE,
                 "Too many RX queues for device; port=%u, rx_queues=%u, "
                 "max_queues=%u \n",
    ...
    {
        rte_exit(EXIT_FAILURE,
                 "Too many TX queues for device; port=%u, tx_queues=%u, "
                 "max_queues=%u \n",
                 port_id, tx_queues, dev_info.max_tx_queues);
        return -EINVAL;
    }

    retval = rte_eth_dev_configure(port_id, nb_rx_queue, tx_queues, &conf);
    if (retval != 0)
    {
    ...
    for (q = 0; q < nb_rx_queue; q++)
    {
        ...
        if (retval != 0)
        {
            //printf("\n");
        printf("port id is %d\n", port_id);
        printf("tx_queues is %d\n", tx_queues);
        printf("nb_rx_desc is %d\n", nb_rx_desc);
        printf("socket is %d\n", socket);
    ...
    {
        retval =
            rte_eth_tx_queue_setup(port_id, q, TX_QUEUE_SIZE, socket, &tx_conf);
        if (retval != 0)
        {
    ...
    LOG_INFO(USER1,
             "Device setup successfully; port=%u, mac=%02" PRIx8 " %02" PRIx8
             " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 "\n",
             (unsigned int)port_id, addr.addr_bytes[0], addr.addr_bytes[1],
 ...
 */
int init_receive(const uint8_t enabled_port_mask, const uint16_t nb_rx_queue,
                 const uint16_t nb_rx_desc)
{
    ...
    nb_ports_available = nb_ports = rte_eth_dev_count_avail();

    // create memory pool
    size = NUM_MBUFS * nb_ports;
    struct rte_mempool *mem_pool =
        rte_pktmbuf_pool_create("mbuf-pool", size, MBUF_CACHE_SIZE, 0,
                                RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
    if (mem_pool == NULL)
    {
        rte_exit(EXIT_FAILURE,
                 "Unable to create memory pool; n=%u, cache_size=%u, "
                 "data_room_size=%u, socket=%u \n",
                 size, MBUF_CACHE_SIZE, RTE_MBUF_DEFAULT_BUF_SIZE,
                 rte_socket_id());
    }
    ...
    {
        rte_exit(EXIT_FAILURE,
                 "Error: No available enabled ports. Portmask set?\n");
    }
 ...
 */
int init_mrtom_rings(struct rte_ring **rings, const unsigned int count, const unsigned int size)
{
    unsigned int i;
    char buf[32];
    ...
    for (i = 0; i < count; i++)
    {
        sprintf(buf, "mrtom-worker-%d", i);
        rings[i] =
            rte_ring_create(buf, size, rte_socket_id(),
                            RING_F_SP_ENQ & RING_F_SC_DEQ); // RING_F_SP_ENQ);
        if (NULL == rings[i])
        {
            rte_exit(EXIT_FAILURE, "Unable to create flow ring: %s \n",
